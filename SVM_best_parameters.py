...
if __name__ == '__main__':
    ...
    # Using Support Vector Machine classifier with linear kernel and several values of parameter C
    classifiers = {
        "Linear SVM (C=0.00001)": CClassifierSVM(C=0.00001, kernel='linear'),
        "Linear SVM (C=0.00005)": CClassifierSVM(C=0.00005, kernel='linear'),
        "Linear SVM (C=0.0001)": CClassifierSVM(C=0.0001, kernel='linear'),
        "Linear SVM (C=0.0005)": CClassifierSVM(C=0.0005, kernel='linear'),
        "Linear SVM (C=0.001)": CClassifierSVM(C=0.001, kernel='linear'),
        "Linear SVM (C=0.005)": CClassifierSVM(C=0.005, kernel='linear'),
        "Linear SVM (C=0.01)": CClassifierSVM(C=0.01, kernel='linear'),
        "Linear SVM (C=0.05)": CClassifierSVM(C=0.05, kernel='linear'),
        "Linear SVM (C=0.1)": CClassifierSVM(C=0.1, kernel='linear'),
        "Linear SVM (C=0.5)": CClassifierSVM(C=0.5, kernel='linear'),
        "Linear SVM (C=1)": CClassifierSVM(C=1, kernel='linear')
    }
    ...
    # Plotting TPR and FNR at false positive rate = 1.0%, as a function of C
    C = [0.00001, 0.00005, 0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1]
    ax_tpr.semilogx(C, tpr_eval)
    ax_tpr.set_title('Linear SVM: TPR (at FPR = 1%)')
    ax_tpr.set_xlabel('C')
    ax_tpr.set_ylabel('True Positive Rate (at FPR = 1%)')
    ax_tpr.set_xticks(C)
    ax_fnr.semilogx(C, fnr_eval)
    ax_fnr.set_title('Linear SVM: FNR (at FPR = 1%)')
    ax_fnr.set_xlabel('C')
